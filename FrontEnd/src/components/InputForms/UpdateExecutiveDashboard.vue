  ...
  <div>
    <form>
      ...
      <div class="p-grid p-fluid">
        <div class="p-col-12 p-md-12">
          ...
          >
            ...
            <update-select-dataset-choice-rule-specific
              v-if="show_dataset_choice_rule_specific"
              @input-dataset="assign_dataset_specific($event)"
              :selected_dataset_specific="selected_dataset_specific"
            ></update-select-dataset-choice-rule-specific>
            <update-select-dataset-choice-rule-recent
              v-if="show_dataset_choice_rule_recent_label"
              @input-label="assign_dataset_recent_label($event)"
              :selected_dataset_label_old="selected_dataset_label"
            ></update-select-dataset-choice-rule-recent>
          </Fieldset>
        </div>
      </div>
      ...
      <div class="p-grid p-fluid">
        <div class="p-col-12 p-md-12">
          <Button
            label="Update ExecutiveDashboard"
            icon="pi pi-plus-refresh"
            iconPos="center"
            @click="update_executive_dashboard"
          />
        </div>
      </div>
      <div v-if="submitted">
...
<script>
import UpdateSelectDatasetChoiceRuleSpecific from "./HelperComponents/UpdateSelectDatasetChoiceRuleSpecific.vue";
import UpdateSelectDatasetChoiceRuleRecent from "./HelperComponents/UpdateSelectDatasetChoiceRuleRecent.vue";

export default {
  props: ["dashboard"],
  components: {
    "update-select-dataset-choice-rule-specific":
      UpdateSelectDatasetChoiceRuleSpecific,
    "update-select-dataset-choice-rule-recent":
      UpdateSelectDatasetChoiceRuleRecent,
  },
  data() {
    return {
      name: "",
  ...
  created() {
    ...
    this.selected_dataset_label = this.dashboard.dataset_label;
    this.selected_visualization_right = this.dashboard.visualization_right;
    this.$axios
      .get("/get_dataset_choice_rules")
      .then((res) => {
        res.data
          .filter(
            (dataset_choice_rule) =>
              dataset_choice_rule.name === this.dashboard.dataset_choice_rule
          )
          .map(
            (dataset_choice_rule) =>
              (this.selected_dataset_choice_rule = {
                dataset_choice_rule: dataset_choice_rule,
              })
          );
      })
      .then(() => {
        this.$axios("/get_dataset/" + this.dashboard.dataset_id)
          .then((res) => {
            const label_prefix = "Dataset Category: ";
            this.selected_dataset_specific.push({
              label: label_prefix + res.data["label"],
              dataset: [
                {
                  dataset_desc:
                    "Dataset: " +
    ...
    this.dashboard.access_user_list.forEach((user_id) => {
      this.$axios.get("/user/" + user_id).then((res) => {
        this.selected_users.push({ user: res.data });
      });
    });

    this.dashboard.access_business_unit_list.forEach((dep) => {
      this.$axios.get("/department_by_name/" + dep).then((res) => {
        if (res.data.name !== "None Department Assigned") {
  ...
  methods: {
    ...
    update_executive_dashboard() {
      ...
      this.formData.append(
        ...
        this.dashboard.executive_dashboard_id
      );
      ...
      this.$axios
        .post(
          "/update_executive_dashboard?uid=" + localStorage.loggedUser,
          this.formData
        )
        .then(() => {
          this.$toast.add({
            severity: "success",
            summary: "Executive Dashboard Update Successful",
            detail: "The Executive Dashboard was updated",
            life: 3000,
          });
          this.close();
        })
        .catch(() => {
          this.$toast.add({
            severity: "error",
            summary: "Executive Dashboard Update Unsuccessful",
            detail: "The Executive Dashboard could not be updated",
            life: 5000,
          });
