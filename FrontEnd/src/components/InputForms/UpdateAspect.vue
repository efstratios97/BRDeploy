<template>
  <div>
    <form>
      ...
      <div
        v-for="raw_component in initial_aspect_raw_components"
        :key="raw_component"
      >
        <update-aspect-helper
        ...
        <div class="p-grid p-fluid">
          <div class="p-col-12 p-md-12">
            <Button
              :label="'Remove Component: ' + Object.keys(raw_component)[0]"
              icon="pi pi-minus-circle"
              iconPos="center"
              class="p-button-danger"
              @click="remove_component(raw_component)"
            />
          </div>
        </div>
        <br />
...
<script>
import UpdateAspectHelper from "./HelperComponents/UpdateAspectHelper.vue";
import AddAspectHelper from "./HelperComponents/AddAspectHelper.vue";
export default {
  components: {
    "add-aspect-helper": AddAspectHelper,
    "update-aspect-helper": UpdateAspectHelper,
  },
  props: ["aspect"],
  data() {
    return {
  ...
  created() {
    this.aspect_id = this.aspect.aspect_id;
    this.name = this.aspect.name;
    this.description = this.aspect.description;
    this.selected_skala_size = this.aspect.skala_size;
    this.initial_aspect_raw_components =
      this.aspect.raw_components_from_dataset;
    this.selected_formula = { formula: this.aspect.formula };
    for (
      let index = 0;
      index < this.aspect.dataset_labels.split(",").length;
      index++
    ) {
      this.$axios
        .get("/get_label_by_id/" + this.aspect.dataset_labels.split(",")[index])
        .then((res) => {
  ...
  methods: {
    ...
    remove_component(raw_component) {
      var initial_aspect_raw_components_tmp = [];
      this.initial_aspect_raw_components.forEach((element) => {
        if (
          Object.keys(element)[0] + Object.values(element)[0] !==
          Object.keys(raw_component)[0] + Object.values(raw_component)[0]
        ) {
          initial_aspect_raw_components_tmp.push(element);
        }
      });
      this.initial_aspect_raw_components = initial_aspect_raw_components_tmp;
      this.$forceUpdate();
    },
    getDatasetOptions() {
      this.$axios
    ...
    update_aspect() {
      ...
      this.formData.append("aspect_id", this.aspect.aspect_id);
      this.formData.append("name", this.name);
      this.formData.append("description", this.description);
      this.formData.append(
        "raw_components_from_dataset",
        JSON.stringify(
          this.initial_aspect_raw_components.concat(
            this.selected_raw_components_operation_types
          )
        )
      );
      ...
      this.$axios
        .post("/update_aspect", this.formData)
        .then(() => {
          this.$toast.add({
            severity: "success",
            summary: "Aspect Update Successful",
            detail: "The Aspect was updated",
            life: 3000,
          });
          this.close();
        })
        .catch(() => {
          this.$toast.add({
            severity: "error",
            summary: "Aspect Update Unsuccessful",
            detail:
